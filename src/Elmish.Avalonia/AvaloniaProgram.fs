...
module AvaloniaProgram =
  ...
  /// Creates a AvaloniaProgram that does not use commands.
  let mkSimple
      (init: unit -> 'model)
  ...
  /// Creates a AvaloniaProgram that uses commands
  let mkProgram
  ...
  let startElmishLoop
    ...
    let updateLogger = program.LoggerFactory.CreateLogger("Elmish.Avalonia.Update")
    let bindingsLogger = program.LoggerFactory.CreateLogger("Elmish.Avalonia.Bindings")
    let performanceLogger = program.LoggerFactory.CreateLogger("Elmish.Avalonia.Performance")
     ...
     * it also helps keep Avalonia in the correct state.
     * https://github.com/elmish/Elmish.Avalonia/issues/371
     * https://github.com/elmish/Elmish.Avalonia/issues/373
     *
  ...
  /// specified using AvaloniaProgram.withLogger.
